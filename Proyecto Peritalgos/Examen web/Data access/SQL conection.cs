...
namespace Examen_web
{
    public class SQL_conection
    {
                    ...
                    try
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                              ...
                              imagen = reader["cimagen"].ToString();
                            }
                        }
                    }
        ...
        public static string numpreg;

        public string JalarPregFilter() 
        {
            using (SqlConnection con = new SqlConnection(GetConnectionString()))
            {

                using (SqlCommand cmd = new SqlCommand(("Use ExamWeb Select * from pregunta" + numpreg), con))
                {
                    cmd.CommandType = System.Data.CommandType.Text;
        ...
        string queryinsert = " (cpreguntas, copcion1, copcion2, copcion3, copcion4, crespuecor, cimagen) values ('";

        public string GuardarPreg() //INCOMPLETO..
        {
            using (SqlConnection con = new SqlConnection(GetConnectionString()))
            {

                using (SqlCommand cmd = new SqlCommand(("Use ExamWeb Insert into pregunta" + numpreg + queryinsert + preg1 + "', '" + op1 + "', '" + op2 + "', '" + op3 + "', '" + op4 + "', '" + resp + "', '" + imagen + "')"), con))
                {
                    cmd.CommandType = System.Data.CommandType.Text;
